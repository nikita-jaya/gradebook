...
3. The map would be something like (I forget map syntax):
   ```r
   map(policy_list, mutate(x$name = x$aggregation(gs_wide, cols = x$assignments)))
   ```
4. I'm not quite sure how to get the output of a given iteration accessible to the next iteration (so that `ps-1` can get passed to `ps`), but the general idea is just to tack columns onto the right side of `gs_wide`.
