...
vignette: >
  %\VignetteIndexEntry{Grade Calculations}
...
## Demo of Version B implementation
...
```{r}
# policy_flat <- policy_nested |>
#     gradebook::flatten_policy()
# 
# gs_wide |>
#     get_category_grades(policy_flat)
```
...
### Version B: For loop over apply

We can tweak the previous version to get rid of `<<-` by using a for-loop.

```{r}
microbenchmark( {
    for (policy_item in policy) {
    gs_wide_b[[policy_item$category]] <- apply(gs_wide_b, 1, agg_assigns, policy_item = policy_item)
}
})
```

This increases the time by 3x, which is probably worth it at this stage of the game. We can come back and speed it up later.

### Version C: For loop over a parallelized apply

*Under construction*. This is an attempt to speed up version B. *Use furrr instead of future_apply*.

```{r}
#| eval: false
microbenchmark( {
    for (policy_item in policy) {
        gs_wide_c[[policy_item$category]] <- future_apply(gs_wide_c, 1, agg_assigns, policy_item = policy_item)
    }
})
...
### Version D: For loop over pmap

*Under construction*: This is an attempt to try using `pmap` on the inner loop.
...
apply_policy <- function(policy_item, gs_wide) {
    gs_wide[[policy_item$category]] <- pmap(gs_wide, agg_assigns, policy_item = policy_item)
}
...
weighted_by_points <- function(assignments, weights, n_drops, ...) {
    ...
    rowsum(assignments * (weights / sum(weights, na.rm = TRUE)), na.rm =TRUE)
}

pol <- policy[[1]]

gs_wide_d |>
    pmap_dbl(weighted_by_points, 
         assignments = pol$assignments,
         weights = paste0(pol$assignments, "_max_pts"),
         n_drops = 0)
```


### Version E: Naive double for loop
